...
已有全息卡可以增加一个独立的手势页面：双手拉开放大并回正面，靠拢缩小，拇指与中指贴合后快速弹开可切换持续转动。鼠标、键盘和按钮仍可使用。

```bash
python scripts/add_gestures.py --project <已有卡片项目>
cd <已有卡片项目>/web-gesture
npm start
```

原卡面、模型和 `web/` 保持不变。默认摄像头关闭，用户开启后才请求权限；画面只在本机处理，不使用麦克风。响指是带“就绪”提示的视觉实验功能，可能漏检或误触发，需要真人试用。目前支持全息路线，尚未接入双图光栅路线。

见[手势控制补充与实操验证](references/gesture-control.md)，包括环境准备、调参位置和浏览器检查。技能包只含代码与文字，识别模型在生成项目的安装阶段下载并校验。
