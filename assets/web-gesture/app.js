...
import {CardMotion} from './gestures/card-motion.js';
import {setupGestures} from './gestures/gesture-ui.js';
...
let renderer,composer,root,face,uniforms,config,dragging=false,gestures;
const motion=new CardMotion();
let motionUIKey,lastProjectionZoom=-1;
...
async function init(){
 ...
 setupControls();gestures=setupGestures({motion,onZoom:setZoom,onHoldZoom:holdZoom,onActivity:setGestureActive,onMotionChange:updateMotionUI,isDragging:()=>dragging});$('gesture-toggle').disabled=false;new ResizeObserver(resize).observe(stage);resize();loading.remove();
 window.__holo={ready:true,config,renderer,root,uniforms,reset,motion,camera,gestures,setGestureActive,modelSource:config.assets.model};renderer.setAnimationLoop(animate);
}
function updateProjection(){const aspect=stage.clientWidth/stage.clientHeight;if(!Number.isFinite(aspect)||aspect<=0)return;const halfH=Math.max(5.65,3.7/aspect)/motion.displayZoom;camera.left=-halfH*aspect;camera.right=halfH*aspect;camera.top=halfH;camera.bottom=-halfH;camera.updateProjectionMatrix();lastProjectionZoom=motion.displayZoom;}
function resize(){const w=stage.clientWidth,h=stage.clientHeight;if(!w||!h||!renderer)return;updateProjection();renderer.setSize(w,h);composer.setSize(w,h);}
function setZoom(value){motion.setZoom(value);$('whole-zoom').textContent=Math.round(motion.zoom*100)+'%';}
function holdZoom(){motion.holdZoom();$('whole-zoom').textContent=Math.round(motion.zoom*100)+'%';}
function setGestureActive(active){const ratio=Math.min(devicePixelRatio,active?1.25:1.75);if(renderer.getPixelRatio()===ratio)return;renderer.setPixelRatio(ratio);composer.setPixelRatio(ratio);resize();}
function updateMotionUI(){
 const key=motion.mode+motion.back;if(key===motionUIKey)return;motionUIKey=key;
 const auto=motion.mode==='sway',spin=motion.mode==='spin';
 $('auto').setAttribute('aria-pressed',String(auto));$('auto').innerHTML=auto?'<span>Ⅱ</span> 暂停赏卡':'<span>▷</span> 自动赏卡';
 $('spin').setAttribute('aria-pressed',String(spin));$('spin').textContent=spin?'暂停转动':'持续转动 ↻';
 $('rotation-status').textContent=spin?'正在轮转正反面':auto?'轻微摇摆中':'转动已暂停';
 $('flip').innerHTML=motion.back?'回到正面 <span>↻</span>':'翻看背面 <span>↻</span>';
 $('view-label').textContent=motion.back?'BACK · 背面':'FRONT · 正面';
}
function manual(){gestures?.manual();}
function reset(){manual();motion.reset();setZoom(1);updateMotionUI();}
function flip(){manual();motion.flip();updateMotionUI();}
function setupControls(){
 ...
 stage.addEventListener('pointerdown',e=>{if(e.button!==0)return;manual();motion.pause();updateMotionUI();dragging=true;last={x:e.clientX,y:e.clientY};stage.setPointerCapture(e.pointerId);stage.focus({preventScroll:true});});
 stage.addEventListener('pointermove',e=>{if(!dragging)return;motion.nudge((e.clientY-last.y)*.005,(e.clientX-last.x)*.006);last={x:e.clientX,y:e.clientY};});
 const up=()=>{if(dragging)manual();dragging=false;};stage.addEventListener('pointerup',up);stage.addEventListener('pointercancel',up);stage.addEventListener('lostpointercapture',up);
 stage.addEventListener('wheel',e=>{e.preventDefault();manual();setZoom(motion.zoom-e.deltaY*.001);},{passive:false});
 stage.addEventListener('keydown',e=>{
  if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','f','F','r','R'].includes(e.key))return;
  e.preventDefault();manual();motion.pause();
  if(e.key==='ArrowLeft')motion.nudge(0,-.07);if(e.key==='ArrowRight')motion.nudge(0,.07);
  if(e.key==='ArrowUp')motion.nudge(-.06,0);if(e.key==='ArrowDown')motion.nudge(.06,0);
  if(e.key.toLowerCase()==='f')flip();if(e.key.toLowerCase()==='r')reset();updateMotionUI();
 });
 $('auto').onclick=()=>{manual();motion.toggleSway();updateMotionUI();};$('flip').onclick=flip;$('reset').onclick=reset;
 $('spin').onclick=()=>{manual();motion.toggleSpin();updateMotionUI();};updateMotionUI();
...
function animate(now){const dt=document.hidden?0:Math.min((now-lastTime)/1000,.1)||0;lastTime=now;elapsed+=dt;
 motion.tick(dt,reduced);if(Math.abs(motion.displayZoom-lastProjectionZoom)>0.00001)updateProjection();root.rotation.set(motion.x,motion.y,0);root.updateMatrixWorld(true);updateMotionUI();
 uniforms.uView.value.copy(camera.position).applyMatrix4(new THREE.Matrix4().copy(root.matrixWorld).invert()).normalize();uniforms.uTime.value=reduced&&motion.mode==='paused'?0:elapsed;composer.render();}
